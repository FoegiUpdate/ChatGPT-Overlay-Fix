...
# Temporarily clears WS_EX_LAYERED on relevant ChatGPT top-level overlay windows,
# forces a native frame refresh, and restores the original extended window style.
# PowerShell 5.1 compatible. No persistent changes are made.

$ErrorActionPreference = 'Stop'

# Define the Win32 helper only once per PowerShell session.
if (-not ('ChatGPTOverlayWin32V21' -as [type])) {
...
public static class ChatGPTOverlayWin32V21
{
...
function Get-WindowInfo {
    ...
    $processId = [uint32]0
    [void][ChatGPTOverlayWin32V21]::GetWindowThreadProcessId($Handle, [ref]$processId)

    $class = New-Object System.Text.StringBuilder 512
    $title = New-Object System.Text.StringBuilder 512

    [void][ChatGPTOverlayWin32V21]::GetClassName($Handle, $class, $class.Capacity)
    [void][ChatGPTOverlayWin32V21]::GetWindowText($Handle, $title, $title.Capacity)

    [ChatGPTOverlayWin32V21]::SetLastError(0)
    $exStylePointer = [ChatGPTOverlayWin32V21]::GetWindowLongPtr(
        $Handle,
        [ChatGPTOverlayWin32V21]::GWL_EXSTYLE
    )
    ...
    $rect = New-Object ChatGPTOverlayWin32V21+RECT
    $width = 0
    $height = 0

    if ([ChatGPTOverlayWin32V21]::GetWindowRect($Handle, [ref]$rect)) {
    ...
    [pscustomobject]@{
        ...
        Width       = $width
        Height      = $height
        Visible     = [ChatGPTOverlayWin32V21]::IsWindowVisible($Handle)
        ExStyleRaw  = $exStyle
        ExStyle     = ('0x{0:X8}' -f $exStyle)
        Transparent = [bool]($exStyle -band [ChatGPTOverlayWin32V21]::WS_EX_TRANSPARENT)
        TopMost     = [bool]($exStyle -band [ChatGPTOverlayWin32V21]::WS_EX_TOPMOST)
        ToolWindow  = [bool]($exStyle -band [ChatGPTOverlayWin32V21]::WS_EX_TOOLWINDOW)
        Layered     = [bool]($exStyle -band [ChatGPTOverlayWin32V21]::WS_EX_LAYERED)
    }
}
...
function Set-ExtendedWindowStyle {
    ...
    # A zero return value from SetWindowLongPtr is not necessarily an error.
    # Clear LastError first and only treat zero as failure when Win32 reports an error.
    [ChatGPTOverlayWin32V21]::SetLastError(0)

    $previousStyle = [ChatGPTOverlayWin32V21]::SetWindowLongPtr(
        $Handle,
        [ChatGPTOverlayWin32V21]::GWL_EXSTYLE,
        [IntPtr]$ExStyle
    )
...
function Invoke-FrameRefresh {
    ...
    $flags = [ChatGPTOverlayWin32V21]::SWP_NOSIZE -bor `
             [ChatGPTOverlayWin32V21]::SWP_NOMOVE -bor `
             [ChatGPTOverlayWin32V21]::SWP_NOZORDER -bor `
             [ChatGPTOverlayWin32V21]::SWP_NOACTIVATE -bor `
             [ChatGPTOverlayWin32V21]::SWP_FRAMECHANGED

    $succeeded = [ChatGPTOverlayWin32V21]::SetWindowPos(
        $Handle,
        [IntPtr]::Zero,
        0,
        0,
...
function Reset-LayeredStyle {
    ...
    $styleWithoutLayered = $originalStyle -band (-bnot [ChatGPTOverlayWin32V21]::WS_EX_LAYERED)
...
# Resolve ChatGPT process IDs first so unrelated Chromium/Electron windows cannot be targeted.
...
[void][ChatGPTOverlayWin32V21]::EnumWindows({
    param($hWnd, $lParam)

    $processId = [uint32]0
    [void][ChatGPTOverlayWin32V21]::GetWindowThreadProcessId($hWnd, [ref]$processId)
...
# Target only visible layered ChatGPT Chromium windows that also expose at least one
# of the known overlay characteristics. This avoids requiring one exact ExStyle value.
$overlayParents = @(
    $topWindows | Where-Object {
        $_.Visible -and
        $_.Class -eq 'Chrome_WidgetWin_1' -and
        $_.Layered -and
        ($_.TopMost -or $_.ToolWindow)
    }
)
...
if ($overlayParents.Count -eq 0) {
    ...
    Write-Host ''
    Write-Host 'Detected ChatGPT top-level windows:'
